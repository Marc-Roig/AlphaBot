...
from src.commands.user import display_calendar, help, list_classes, make_booking, cancel_booking
from src.commands.user.conversation_handlers import set_token, aimh_login as login
...
def add_user_commands(app: Application) -> None:
    ...
    add_handler(
        CH(
            entry_points=[CMH("token", set_token.start)],
            states={
                set_token.START: [MGH(Regex("^(.*?)+$"), set_token.set_token)],
            },
    ...
    add_handler(
        CH(
            entry_points=[
                CMH("login", login.start),
                CQH(login.start, pattern="login")
            ],
            states={
                login.START: [MGH(Regex("^(.*?)+$"), login.login)],
            },
...
def add_repeating_jobs(app: Application) -> None:
    ...
    # Get time remaining seconds until the start of next minute plus a margin
    seconds_until_next_minute = 60 - datetime.datetime.now().second + 0.5
